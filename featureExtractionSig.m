function [mtFeatures, stFeaturesPerSegment] = ...
    featureExtractionSig(signal, stWin, stStep, featureStatistics)

% function [midFeatures, stFeaturesPerSegment] = ...
%    featureExtractionFile(fileName, stWin, stStep, ...
%    featureStatistics)
%
...
%while (curSample <= numOfSamples) % while the end of file has not been reahed
   ...
   % Features = stFeatureExtraction(tempX, fs, stWin, stStep); -old
     Features = stFeatureExtraction(signal, 44000, stWin, stStep);  
